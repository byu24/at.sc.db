...
while [ "$i" -lt "$len" ]; do
  ...
  echo "nohup R CMD BATCH $BSCRATCH/at.sc.db/src/analyze/${lib_name}_analyze.r BSCRATCH/at.sc.db/log/${lib_name}_analyze.Rout &" >> src/analyze/launch_analyze_scripts.sh
